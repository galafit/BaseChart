...
public interface ShortSet {
    ...
    public default int lowerBound(int fromIndex, int length, double doubleValue) {
        short value = new Double(Math.floor(doubleValue)).shortValue();
        int low = fromIndex;
        ...
        while (low <= high) {
            ...
            if (value > get(mid)) {
                ...
                if(value == doubleValue) {
                    high = mid - 1; // если doubleValue на самом деле short и есть равные ему элементы то бежим по ним вниз
                } else {
    ...
    public default int upperBound(int fromIndex, int length, double doubleValue) {
        short value = new Double(Math.ceil(doubleValue)).shortValue();
        int low = fromIndex;
        int high = fromIndex + length -1;
        int index = -1;
        while (low <= high) {
            ...
            if (value > get(mid)) {
                ...
                index = mid;
                if(value == doubleValue) {
                    low = low + 1; // если doubleValue на самом деле short и есть равные ему элементы то бежим по ним вверх
                } else {
